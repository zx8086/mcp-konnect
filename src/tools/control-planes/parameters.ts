...
export const createControlPlaneParameters = () => z.object({
  name: z.string()
    .min(1)
    .describe("Control plane name"),
  description: z.string()
    .optional()
    .describe("Control plane description"),
  clusterType: z.enum(["CLUSTER_TYPE_HYBRID", "CLUSTER_TYPE_K8S_INGRESS_CONTROLLER", "CLUSTER_TYPE_CONTROL_PLANE"])
    .default("CLUSTER_TYPE_HYBRID")
    .describe("Cluster type for the control plane"),
  cloudGateway: z.boolean()
    .default(false)
    .describe("Enable cloud gateway for this control plane"),
  authType: z.enum(["pki", "pinned_certs"])
...
export const updateControlPlaneParameters = () => z.object({
  controlPlaneId: CommonSchemas.uuid
    .describe("Control Plane ID (obtainable from list-control-planes tool)"),
  name: z.string()
    .optional()
    .describe("New control plane name"),
  description: z.string()
    .optional()
...
export const deleteControlPlaneParameters = () => z.object({
  controlPlaneId: CommonSchemas.uuid
    .describe("Control Plane ID (obtainable from list-control-planes tool)")
});
...
export const listDataPlaneNodesParameters = () => z.object({
  ...
  pageNumber: z.number().int()
    ...
    .optional()
    .describe("Page number to retrieve"),
  filterStatus: z.enum(["connected", "disconnected", "partially_connected"])
    .optional()
    .describe("Filter nodes by connection status"),
  filterHostname: z.string()
    .optional()
...
export const getDataPlaneNodeParameters = () => z.object({
  ...
  nodeId: CommonSchemas.uuid
    .describe("Data plane node ID")
});

export const createDataPlaneTokenParameters = () => z.object({
  controlPlaneId: CommonSchemas.uuid
    .describe("Control Plane ID (obtainable from list-control-planes tool)"),
  name: z.string()
    .describe("Name for the data plane token"),
  expiresAt: z.string()
    .optional()
    .describe("Token expiration time (ISO 8601 format)")
});

export const revokeDataPlaneTokenParameters = () => z.object({
  controlPlaneId: CommonSchemas.uuid
    .describe("Control Plane ID (obtainable from list-control-planes tool)"),
  tokenId: CommonSchemas.uuid
    .describe("Data plane token ID to revoke")
});

export const listDataPlaneTokensParameters = () => z.object({
  controlPlaneId: CommonSchemas.uuid
    .describe("Control Plane ID (obtainable from list-control-planes tool)"),
  pageSize: z.number().int()
    .min(1).max(1000)
...
export const getControlPlaneConfigParameters = () => z.object({
  controlPlaneId: CommonSchemas.uuid
    .describe("Control Plane ID (obtainable from list-control-planes tool)")
});

export const updateControlPlaneConfigParameters = () => z.object({
  controlPlaneId: CommonSchemas.uuid
    .describe("Control Plane ID (obtainable from list-control-planes tool)"),
  proxyUrl: z.string()
    .optional()
    .describe("Proxy URL for the control plane"),
  telemetryUrl: z.string()
    .optional()
    .describe("Telemetry endpoint URL"),
  authType: z.enum(["pki", "pinned_certs"])
    .optional()
