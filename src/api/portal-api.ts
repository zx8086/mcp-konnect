...
export class PortalApi {
  ...
  async listApplications(pageSize = 10, pageNumber?: number, filterName?: string, filterAuthStrategy?: string): Promise<any> {
  ...
  async listApplicationRegistrations(applicationId: string, pageSize = 10, pageNumber?: number, filterStatus?: string, filterApiName?: string): Promise<any> {
  ...
  async updateCredential(applicationId: string, credentialId: string, credentialData: any): Promise<any> {
    return this.portalRequest<any>(`/api/v3/applications/${applicationId}/credentials/${credentialId}`, "PATCH", credentialData);
  }
