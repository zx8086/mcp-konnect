...
import { createTracedToolHandler, BatchToolTracer } from '../utils/tool-tracer.js';
...
describe('Enhanced Conversation-Aware Tracing', () => {
  ...
  describe('Conversation Tracking System', () => {
    it('should create new conversation with proper ID generation', () => {
      const conversation = getOrCreateConversation(testSessionId, 'list_control_planes');
    ...
    it('should continue existing conversation and increment message count', () => {
      ...
      const conv1 = getOrCreateConversation(testSessionId, 'list_control_planes');
      ...
      expect(conv2.toolSequence).toEqual(['list_control_planes', 'get_control_plane']);
    });
    ...
    it('should create conversation-aware trace names correctly', () => {
      const conversation1 = getOrCreateConversation(testSessionId, 'query_api_requests');
      const traceName1 = createConversationAwareTraceName('query_api_requests', conversation1);
      expect(traceName1).toMatch(/^query_api_requests \[NEW:[a-z0-9]{6}\]$/);

      const conversation2 = getOrCreateConversation(testSessionId, 'get_consumer_requests');
      const traceName2 = createConversationAwareTraceName('get_consumer_requests', conversation2);
    ...
    it('should track conversation flow patterns', () => {
      ...
      const conversation = getOrCreateConversation(testSessionId, 'create_route');
  ...
  describe('Intent Detection System', () => {
    it('should detect exploration intent from list operations', () => {
      ...
      expect(intent.evidence).toContain("Tool 'list_control_planes' matches exploration pattern");
    });

    it('should detect configuration intent from create operations', () => {
      const intent = detectIntent('create_service', { name: 'test-service', host: 'api.test.com' }, []);
    ...
    it('should detect API gateway setup pattern from tool sequence', () => {
      ...
      expect(intent.evidence.some(e => e.includes('conversation pattern'))).toBe(true);
    });

    it('should detect security management from certificate operations', () => {
      const intent = detectIntent('create_certificate', { 
      ...
      expect(intent.confidence).toBeGreaterThan(0.7);
      expect(intent.evidence.some(e => e.includes('security_management'))).toBe(true);
    });

    it('should detect context transitions between intents', () => {
      const previousTools = ['list_control_planes', 'list_services'];
      const transition = detectContextTransition(previousTools, 'query_api_requests', 'exploration');
    ...
    it('should provide intent-based suggestions', () => {
      ...
      const suggestions = getIntentBasedSuggestions(intent, ['create_service'], {});
  ...
  describe('Conversation Quality Metrics', () => {
    it('should calculate conversation quality scores', () => {
      const mockConversation: ConversationInfo = {
        ...
        toolSequence: ['list_control_planes', 'get_control_plane', 'list_services', 'create_service', 'create_route'],
    ...
    it('should calculate conversation statistics', () => {
      const mockConversation: ConversationInfo = {
        ...
        messageCount: 3,
        isNewConversation: false,
        toolSequence: ['query_api_requests', 'get_consumer_requests', 'list_services'],
    ...
    it('should generate conversation insights', () => {
      const mockConversation: ConversationInfo = {
        ...
        messageCount: 5,
        isNewConversation: false,
        toolSequence: ['list_control_planes', 'get_control_plane', 'create_service', 'create_route', 'create_plugin'],
        startTime: Date.now() - 300000, // 5 minutes ago
        lastActivity: Date.now(),
      ...
      const insights = generateConversationInsights(mockQuality, mockConversation, 'configuration');
  ...
  describe('Enhanced Session Management', () => {
    it('should create session context with conversation fields', () => {
      const context = createSessionContext('test-conn-123', 'stdio', testSessionId, 
    ...
    it('should detect session resumption after gap', () => {
      const context = createSessionContext('test-conn-456', 'stdio', testSessionId);
    ...
    it('should update session with conversation context', () => {
      const context = createSessionContext('test-conn-789', 'stdio', testSessionId);
      
      runWithSession(context, () => {
        updateSessionWithConversation('create_service', 'configuration', 'api_gateway');
    ...
    it('should track topic transitions', () => {
      const context = createSessionContext('test-conn-101', 'stdio', testSessionId);
      
      runWithSession(context, () => {
        updateSessionWithConversation('list_control_planes', 'exploration', 'discovery');
        updateSessionWithConversation('query_api_requests', 'analysis', 'performance');
    ...
    it('should generate conversation summary', () => {
      const context = createSessionContext('test-conn-202', 'stdio', testSessionId);
  ...
  describe('Enhanced Tool Tracing', () => {
    it('should create traced tool handler with conversation context', async () => {
      ...
      const tracedHandler = createTracedToolHandler(mockHandler, 'test_tool', mockTracingManager);
      
      const context = createSessionContext('test-conn-303', 'stdio', testSessionId);
      
      await runWithSession(context, async () => {
        const result = await tracedHandler({ testParam: 'value' }, { requestId: 'req-123' });
        expect(result).toBeDefined();
        expect(mockHandler).toHaveBeenCalledWith({ testParam: 'value' }, { requestId: 'req-123' });
      });
    });
    ...
    it('should create batch tool tracer with conversation awareness', async () => {
      ...
      const context = createSessionContext('test-conn-404', 'stdio', testSessionId);
      ...
      await runWithSession(context, async () => {
        ...
        const results = await batchTracer.traceToolBatch('create_service', operations);
  ...
  describe('Integration Tests', () => {
    it('should integrate all conversation components in realistic flow', async () => {
      const context = createSessionContext('integration-test', 'stdio', 'integration-session');
      
      await runWithSession(context, async () => {
        // 1. Start with exploration
        let conversation = getOrCreateConversation('integration-session', 'list_control_planes');
        expect(conversation.isNewConversation).toBe(true);
        
        // 2. Continue with detailed exploration
        conversation = getOrCreateConversation('integration-session', 'get_control_plane');
        expect(conversation.messageCount).toBe(2);
        
        // 3. Move to configuration
        conversation = getOrCreateConversation('integration-session', 'create_service');
        const intent = detectIntent('create_service', { name: 'test' }, conversation.toolSequence);
        expect(intent.primary).toBe('configuration');
        ...
        updateSessionWithConversation('create_route', 'configuration', 'api_setup');
    ...
    it('should handle conversation timeout and create new conversation', async () => {
      const context = createSessionContext('timeout-test', 'stdio', 'timeout-session');
      
      await runWithSession(context, async () => {
        // Create first conversation
        const conv1 = getOrCreateConversation('timeout-session', 'list_services');
        ...
        const conv2 = getOrCreateConversation('timeout-session', 'create_consumer');
