...
describe('Flight API Integration Tests', () => {
  ...
  describe('Complete Flight Booking Workflow', () => {
    test('should complete end-to-end flight booking', async () => {
      const authHeaders = { 'X-API-Key': TEST_FIXTURES.authCredentials.keyAuth };

      // Step 1: Search for flights
      const searchResponse = await testUtils.simulateApiRequest('GET', '/flights?origin=JFK&destination=LAX', authHeaders);
