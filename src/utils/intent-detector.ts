 ...
 */
function analyzeToolSequence(tools: string[]): { intent: string; confidence: number } {
  ...
  if (recent.some(tool => tool.includes('query')) && 
...
function analyzeToolArguments(toolName: string, args: any): { intent: string; confidence: number } {
  ...
  const filterCount = Object.keys(args).filter(key => 
    key.startsWith('filter') || key.includes('Filter') || key.includes('search')
  ).length;
