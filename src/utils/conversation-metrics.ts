...
import type { ConversationInfo, ConversationFlow } from './conversation-tracker.js';
 ...
 */
function calculateCoherenceScore(conversation: ConversationInfo, flow?: ConversationFlow): number {
 ...
 */
function identifySatisfactionIndicators(conversation: ConversationInfo): string[] {
  const indicators: string[] = [];
  const { errorCount, messageCount, averageResponseTime, toolSequence } = conversation;
 ...
 */
function calculateCompleteness(conversation: ConversationInfo, intent?: UserIntent): number {
  ...
  switch (intent.primary) {
    ...
    case 'configuration':
      ...
      const hasValidation = toolSequence.some(tool => tool.includes('get_') || tool.includes('list'));
      completeness = hasCreate ? (hasValidation ? 0.95 : 0.8) : 0.4;
      break;
      
    case 'analysis':
      // Should have query operations and possibly follow-up
      const hasQuery = toolSequence.some(tool => tool.includes('query') || tool.includes('analytics'));
    ...
    default:
      ...
      const diversity = new Set(toolSequence).size / Math.min(toolSequence.length, 10);
 ...
 */
function assessEngagementLevel(conversation: ConversationInfo): 'low' | 'medium' | 'high' {
 ...
 */
function assessComplexityLevel(conversation: ConversationInfo): 'simple' | 'moderate' | 'complex' {
...
function calculateUserExperienceScore(factors: {
  ...
  const { coherenceScore, efficiencyScore, completeness, performance } = factors;
...
export function generateConversationInsights(
  ...
  const recommendedImprovements = generateRecommendations(quality, conversation);
...
function identifyLogicalProgressions(toolSequence: string[]): number {
  ...
  for (let i = 0; i < toolSequence.length - 1; i++) {
    ...
    if (current.includes('create') && (next.includes('get_') || next.includes('list'))) {
      progressions++;
    }
...
function calculateRandomness(toolSequence: string[]): number {
  ...
  for (let i = 0; i < toolSequence.length - 1; i++) {
    ...
    const hasCommonWords = currentWords.some(word => nextWords.includes(word));
...
function hasCompletionIndicators(toolSequence: string[]): boolean {
  ...
  const completionPatterns = [
    ...
    (seq: string[]) => seq.some(t => t.includes('create')) && seq.some(t => t.includes('get_')),
...
function showsEfficientProblemSolving(toolSequence: string[]): boolean {
  ...
  return toolSequence.length >= 3 && 
         toolSequence.length <= 6 && // Not too many tools
         new Set(toolSequence).size === toolSequence.length; // No repetition
}
...
function hasAdvancedToolUsage(toolSequence: string[]): boolean {
  const advancedTools = ['query_api_requests', 'create_certificate', 'update_plugin', 'create_portal'];
...
function generateRecommendations(quality: ConversationQuality, conversation: ConversationInfo): string[] {
  ...
  if (quality.efficiencyScore < 0.7) {
    ...
    recommendations.push('Use list operations to explore before diving into details');
  }
  ...
  if (conversation.averageResponseTime > 3000) {
    recommendations.push('Consider using filters to narrow down large result sets');
  }
  
  return recommendations;
}

function assessSkillLevel(conversation: ConversationInfo): 'beginner' | 'intermediate' | 'advanced' {
  ...
  if (uniqueTools > 4 || (hasAdvancedToolUsage(toolSequence) && errorCount < 2)) {
    return 'intermediate';
  }
