...
export type ClientType = 
  ...
  // IDE Integrations  
  | 'vscode-extension' | 'jetbrains-plugin' | 'vim-plugin' | 'emacs-extension'
  // MCP Client Libraries
  | 'mcp-client-python' | 'mcp-client-typescript' | 'mcp-client-rust' | 'mcp-client-go'
  // Custom/Enterprise Clients
  | 'custom-web-client' | 'custom-desktop-client' | 'enterprise-client' | 'api-client'
  // Development Tools
  | 'cli-tool' | 'automation-script' | 'testing-framework' | 'ci-cd-pipeline'
  // Unknown/Fallback
  | 'unknown';
...
function runOfficialClientDetection(
  ...
  if (env.CLAUDE_CODE_VERSION || env.CLAUDE_CLI_VERSION || 
...
function runIDEIntegrationDetection(
  ...
  if (cwd.includes('WebstormProjects') || cwd.includes('IntellijProjects') || 
      env.JETBRAINS_IDE || argv.some(arg => arg.includes('jetbrains'))) {
    results.push({
...
function runMCPLibraryDetection(
  ...
  // Python MCP Client
  if (argv.some(arg => arg.includes('python')) && 
  ...
  if ((argv.some(arg => arg.includes('node') || arg.includes('bun') || arg.includes('tsx'))) && 
