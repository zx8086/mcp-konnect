...
export function getOrCreateConversation(sessionId: string, toolName?: string): ConversationInfo {
  ...
  const isExpired = conversation && (now - conversation.lastActivity) > CONVERSATION_TIMEOUT;
...
export function updateConversation(
  ...
  conversation.averageResponseTime = conversation.totalExecutionTime / conversation.messageCount;
  
  // Detect context switches
  if (context && conversation.lastContext && context !== conversation.lastContext) {
 ...
 */
export function createConversationAwareTraceName(toolName: string, conversationInfo: ConversationInfo): string {
 ...
 */
export function detectConversationFlow(toolSequence: string[]): ConversationFlow | null {
  ...
  if (recent.some(tool => tool.includes('search') || tool.includes('query')) &&
  ...
  if (recent.some(tool => tool.includes('list')) &&
  ...
  if (recent.some(tool => tool.includes('create')) &&
  ...
  if (recent.some(tool => tool.includes('list') || tool.includes('get_')) &&
  ...
  if (new Set(recent).size > 3) { // Diverse tool usage
    return {
      type: 'EXPLORATORY',
 ...
 */
function detectTopicFromTool(toolName: string, context?: string): string | null {
 ...
 */
export function trackConversationFlow(toolName: string, context?: string): void {
