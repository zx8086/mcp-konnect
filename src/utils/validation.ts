...
export const CommonSchemas = {
  ...
  hostname: z.string().regex(/^[a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?)*$/, "Invalid hostname format"),
...
export const timeRangeSchema = z.enum(["15M", "1H", "6H", "12H", "24H", "7D"], {
  errorMap: () => ({ message: "Time range must be one of: 15M, 1H, 6H, 12H, 24H, 7D" })
});

/**
 * Certificate validation
 */
export function validateCertificate(cert: string): { isValid: boolean; error?: string } {
...
export const hashOnSchema = z.enum([
  "none", "consumer", "ip", "header", "cookie", "path", "query_arg", "uri_capture"
], {
...
export const FieldValidations = {
  ...
  nonEmptyArray: <T>(schema: z.ZodType<T>, fieldName: string) => 
    z.array(schema)
      .min(1, `${fieldName} must have at least one element`),
 ...
 */
export function validateJsonConfig(config: any, configName = "configuration"): { isValid: boolean; sanitized?: any; error?: string } {
  try {
