...
export function formatEntity(entity: any, entityType: string): string {
  const formatted = [`${entityType.toUpperCase()}: ${entity.name || entity.id}`];
  ...
  switch (entityType) {
    ...
    case 'route':
      if (entity.methods) formatted.push(`Methods: ${entity.methods.join(', ')}`);
      if (entity.hosts) formatted.push(`Hosts: ${entity.hosts.join(', ')}`);
      if (entity.paths) formatted.push(`Paths: ${entity.paths.join(', ')}`);
      if (entity.protocols) formatted.push(`Protocols: ${entity.protocols.join(', ')}`);
      break;
    ...
    case 'plugin':
      ...
      if (entity.consumer?.id) formatted.push(`Consumer: ${entity.consumer.id}`);
      break;
    ...
    case 'data-plane-node':
      ...
      if (entity.last_seen) formatted.push(`Last Seen: ${formatTimestamp(entity.last_seen)}`);
      break;
  }
...
export function formatEntitySummary(entity: any, entityType: string): string {
  ...
  switch (entityType) {
    case 'service':
      if (entity.host) parts.push(`(${entity.protocol || 'http'}://${entity.host}:${entity.port || 80})`);
      break;
    
    case 'route':
      const routeInfo = [];
      if (entity.methods) routeInfo.push(`Methods: ${entity.methods.join(',')}`);
    ...
    case 'consumer':
      if (entity.username && entity.custom_id) {
        parts.push(`(Username: ${entity.username}, Custom ID: ${entity.custom_id})`);
...
export function formatApiRequestsResponse(response: any): string {
  ...
  results.forEach((request: any) => {
    ...
    if (request.gateway_service) {
      serviceCounts[request.gateway_service] = (serviceCounts[request.gateway_service] || 0) + 1;
    }
...
export function formatHealthStatus(health: any, resourceName: string): string {
  ...
  if (health.targets && Array.isArray(health.targets)) {
    ...
    health.targets.forEach((target: any, index: number) => {
      ...
      status.push(`  ${index + 1}. ${target.target || 'Unknown'} - Status: ${targetStatus}, Weight: ${weight}`);
    });
  }
...
export function formatCertificate(cert: any): string {
  ...
  if (cert.cert) {
    try {
      ...
      const certData = certLines.find((line: string) => line.startsWith('-----BEGIN CERTIFICATE-----'));
      if (certData) {
