import { z } from "zod";
import { ElicitationManager, KongElicitationPatterns, ElicitationRequest } from "../utils/elicitation.js";
...
export class MigrationAnalyzer {
  ...
  async analyzeMigration(context: MigrationContext): Promise<MigrationAnalysis> {
    const analysis: MigrationAnalysis = {
      ...
      team: null,
      controlPlane: null,
      entityCounts: { services: 0, routes: 0, consumers: 0, plugins: 0, total: 0 },
    ...
    analysis.missingInfo = {
      domain: !analysis.domain || analysis.domain.confidence < MigrationAnalyzer.CONFIDENCE_THRESHOLDS.MINIMUM_ACCEPTABLE,
      environment: !analysis.environment || analysis.environment.confidence < MigrationAnalyzer.CONFIDENCE_THRESHOLDS.MINIMUM_ACCEPTABLE,
   ...
   */
  async createElicitationSession(analysis: MigrationAnalysis, context: MigrationContext): Promise<{
    sessionId: string;
  ...
  private extractDomainInfo(context: MigrationContext): ExtractedInfo | null {
    const candidates: { value: string; confidence: number; source: ExtractedInfo['source']; sources: string[] }[] = [];
  ...
  private countEntities(deckConfigs: DeckConfiguration[]) {
    const counts = { services: 0, routes: 0, consumers: 0, plugins: 0, total: 0 };
